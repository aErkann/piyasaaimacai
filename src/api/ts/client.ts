...
export async function backendRequest<T>(config: ApiRequestConfig): Promise<ApiResponse<T>> {
  ...
  try {
    ...
    return { success: true, data: data as T, error: null, timestamp: new Date().toISOString() };
  } catch (err) {
...
// --- CoinGecko ---
export async function coingeckoFetch(endpoint: string, params?: Record<string, string>) {
...
// --- DEX Screener ---
export async function dexScreenerFetch(endpoint: string) {
...
// --- API-Football ---
export async function apiFootballFetch(endpoint: string, params?: Record<string, string>) {
...
// --- OpenAI ---
export async function openAIFetch(prompt: string, systemPrompt?: string) {
...
// --- Gemini ---
export async function geminiFetch(prompt: string) {
